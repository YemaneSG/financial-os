...
| PM-EDGE-01 | Every owner-triggered Edge Function first evaluates the common owner predicate using the caller JWT. It may use an administrative/server client only after success. The synthetic test subject is never added to the receipt API allowlist. | Hosted Link function authorization matrix and service-role-before-auth negative test |
